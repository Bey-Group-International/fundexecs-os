-- Migration: Cross-Session Agent Memory (Feature 04) and Session Memory Cards (Feature 06).
--
-- Feature 04: agent_memories table stores persistent agent knowledge across sessions —
--   decisions, constraints, preferences, outcomes, and open items — with optional
--   pgvector embeddings for semantic retrieval.
-- Feature 06: extends sessions with a memory_card jsonb column for lightweight
--   per-session structured summaries surfaced in the UI.
...
-- ── sessions.memory_card (Feature 06) ────────────────────────────────────────
