// The follow-up endpoint gathers an org-scoped meeting + its linked deal/fund and
// any saved report notes, and returns a composed institutional follow-up prompt.
...
const req = () => new NextRequest("http://localhost/api/meetings/m1/followup");
...
function wire(tables: { live_meetings?: unknown; deals?: unknown; funds?: unknown; live_meeting_reports?: unknown }) {
...
describe("GET /api/meetings/[id]/followup", () => {
  it("composes a prompt from meeting + deal + fund + saved notes", async () => {
    wire({
      live_meetings: { title: "LP Update", objective: "Confirm re-up", deal_id: "d1", related_fund_id: null, attendees: [{ name: "Jane", type: "external" }] },
      ...
      live_meeting_reports: { summary: "LP supportive of re-up", key_points: ["Ahead of plan"], action_items: ["Sam: Send deck"] },
    });
    const res = await GET(req(), params);
    expect(res.status).toBe(200);
    const { prompt } = await res.json();
    expect(prompt).toContain("LP Update");
    expect(prompt).toContain("Confirm re-up");
    expect(prompt).toContain("Atlas Logistics");
    expect(prompt).toContain("Fund III");
    expect(prompt).toContain("Jane");
    expect(prompt).toContain("LP supportive of re-up");
    expect(prompt).toContain("Sam: Send deck");
    // The follow-up outline is present.
    expect(prompt).toContain("Follow-up email");
    expect(prompt).toContain("Proposed next meeting");
  });

  it("works with a bare meeting (no linked records or notes)", async () => {
    wire({ live_meetings: { title: "Quick sync", deal_id: null, related_fund_id: null, attendees: null }, live_meeting_reports: null });
    const res = await GET(req(), params);
    expect(res.status).toBe(200);
    const { prompt } = await res.json();
