    ...
    return (
      <div className="rounded-2xl border border-[rgba(52,211,153,0.25)] bg-[rgba(52,211,153,0.07)] p-8 text-center">
        <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-[rgba(52,211,153,0.15)]">
          <svg viewBox="0 0 24 24" fill="none" className="h-6 w-6 text-[#34d399]" stroke="currentColor" strokeWidth={2}>
    ...
    <form ref={formRef} action={action} className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          >
            <option value="" disabled className="bg-[#0d1424]">Select stage</option>
            {STAGES.map((s) => (
              <option key={s.value} value={s.value} className="bg-[#0d1424]">{s.label}</option>
            ))}
          </select>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Field label="Founder email *" name="founder_email" type="email" placeholder="jane@acme.io" />
      </div>
      ...
      <p className="text-center text-[11.5px] text-[#7a899e]">
        ...
        <a href="/terms" className="underline hover:text-[#94a3b8]">Terms</a> and{' '}
        <a href="/privacy" className="underline hover:text-[#94a3b8]">Privacy Policy</a>.
      </p>
    </form>
  );
}
