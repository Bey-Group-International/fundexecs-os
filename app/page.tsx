...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      {/* Top nav */}
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/5 bg-[#0a0a0a]/80 backdrop-blur-sm">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="rounded-md bg-agent-associate px-3 py-1.5 text-sm font-medium text-white transition hover:opacity-90"
            >
              Request access
            </Link>
          </div>
      ...
      <section className="mx-auto max-w-6xl px-6 pb-24 pt-40">
        <div className="max-w-3xl">
          ...
          <div className="mt-8 flex gap-3">
            <Link
              href="/login?mode=signup"
              className="rounded-md bg-agent-associate px-5 py-2.5 text-sm font-medium text-white transition hover:opacity-90"
            >
              Request access
            </Link>
            <Link
              href="/login"
              className="rounded-md border border-white/10 px-5 py-2.5 text-sm text-neutral-300 transition hover:bg-white/5"
            >
              Sign in
            </Link>
          </div>
        ...
        {/* Stat strip */}
        <div className="mt-16 grid grid-cols-3 gap-px overflow-hidden rounded-xl border border-white/5 bg-white/5">
          {[
      ...
      <section className="mx-auto max-w-6xl px-6 py-24">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2">
          {HUBS.map((hub, i) => (
            <div
              key={hub.key}
              className="rounded-xl border border-white/5 bg-white/[0.02] p-6 transition hover:border-agent-associate/30 hover:bg-white/[0.04]"
            >
              <div className="flex items-start justify-between">
                <div>
                  <span className="font-mono text-lg text-agent-associate">
                    {HUB_ICONS[hub.key]}
                  </span>
      ...
      <section className="mx-auto max-w-6xl px-6 py-24 text-center">
        ...
        <Link
          href="/login?mode=signup"
          className="mt-8 inline-block rounded-md bg-agent-associate px-6 py-3 text-sm font-medium text-white transition hover:opacity-90"
        >
      ...
      <footer className="border-t border-white/5 px-6 py-6">
        <div className="mx-auto flex max-w-6xl items-center justify-between">
          <span className="font-mono text-xs text-neutral-700">FundExecs OS · Pre-Alpha</span>
