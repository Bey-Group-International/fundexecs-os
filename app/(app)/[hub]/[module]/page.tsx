...
export default async function ModulePage(
  ...
  const needsCapitalMap =
    (params.hub === "source" && params.module === "lp_pipeline") ||
    (params.hub === "run" && params.module === "outreach");
  ...
  if (needsCapitalMap || needsGPScorecard || isReporting || isDealPipeline) {
    ...
    // Run › Outreach — GPLPMatch priority queue above OutreachStudio.
    if (params.hub === "run" && params.module === "outreach") {
      const capitalMap = ctx?.orgId ? await buildCapitalMap(supabase) : [];
      const now = Date.now();
  ...
  // Build › Thesis — Sector Heatmap. The Deal Signal Feed lives on /deals/feed;
  // ThesisLive loads heatmap snapshots for the active org (migration 0058),
  // best-effort, and renders the heatmap with a link out to the feed.
