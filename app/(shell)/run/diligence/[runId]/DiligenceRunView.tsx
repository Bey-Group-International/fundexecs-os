...
import { convictionTone, scoreColor, statusLabel, statusTone } from '@/lib/diligence-ui';
import type { DiligenceAnalystFinding, DiligenceRunDetail } from '@/lib/queries/diligence';
import { cn } from '@/lib/utils';

function ScoreBar({ value, label }: { value: number; label: string }) {
  // The shared ProgressBar's tones don't cover the red/amber/green score read,
  // so this bar colors by score band directly.
  return (
    <div
...
function AnalystCard({ finding }: { finding: DiligenceAnalystFinding }) {
  ...
  return (
    <Card className="p-4">
      ...
      {finding.score != null ? (
        <div className="mt-3">
          <ScoreBar value={finding.score} label={`${finding.laneLabel} score`} />
        </div>
      ) : null}
  ...
  return (
    <div className="flex flex-col gap-[18px]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-[10.5px] font-semibold uppercase tracking-[0.11em] text-fg-4">
            Diligence Intelligence Layer
          </div>
          <h1 className="mt-1 text-[19px] font-semibold tracking-[-0.015em] text-fg-1">
      ...
      {synthesis ? (
        <Card className="bg-[linear-gradient(100deg,rgba(247,201,72,0.08),transparent_58%)] p-5">
          ...
          {synthesis.conviction != null ? (
            <div className="mt-4">
              <ScoreBar value={synthesis.conviction} label="Conviction" />
            </div>
          ) : null}
      ...
      {run.analysts.length > 0 ? (
        <div>
          <div className="mb-3">
            ...
            <h2 className="mt-1 text-[15px] font-semibold tracking-[-0.01em] text-fg-1">
              Analyst findings
            </h2>
          </div>
