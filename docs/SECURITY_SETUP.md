...
| `.github/rulesets/main-branch-protection.json` | Importable branch ruleset: PR required (0 approvals, no Code-Owner gate — solo-maintainer friendly), dismiss-stale, conversation resolution, required checks (`Typecheck, lint & build`, `Playwright e2e smoke`), linear history, block force-push/deletion, squash-only | Import in step 2                                                        |
