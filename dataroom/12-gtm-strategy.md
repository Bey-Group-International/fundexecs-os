...
Enter at the moment of maximum pain and budget: **capital formation for emerging managers**. Prove the product on an in-house design partner (BGI Fund I) and a small cohort of external design partners, sell founder-to-founder through the founder's operating network and capital-formation salons, let the product demonstrate itself (demo seed + guided tour + credit packs), and expand from the raise workflow into the full lifecycle — because once the raise lives in FundExecs OS, the deals, diligence, and reporting follow naturally.
...
## 4. Sales motion and funnel model

```
Salon / referral / content → signup (free demo org)
  → activation: first approved workflow with a real deliverable   [aha moment]
  → credit pack purchase → plan (Starter/Pro)
  → expansion: automations trusted + seats added → Scale
```

Planning ratios (illustrative): salon of 50 → ~15 signups → ~8 activated → ~4 paying; referral K-factor target ≥0.4 by Phase 2. CAC target <$1.5K blended in Phases 1–2 (founder-led time being the true cost), payback <12 months at repriced ARPA.
...
- **Category:** *the AI-native operating system for private capital* (not "CRM," not "copilot").
- **Primary message:** *Fifteen AI executives that own the work. Nothing reaches a counterparty without your sign-off.*
- **Proof style:** live demo over deck — "Source multifamily targets in Texas under $50M" → plan → approve → deliverables + a pipeline deal, in minutes.
- **Against incumbents:** "Your tools hold information. Your OS does the work."
