...
const ROOM_GRID: Record<string, { col: number; row: number }> = {
  ceo:       { col: 0, row: 0 },
...
const NPC_DEFS: Array<{ id: string; name: string; spriteKey: string; roomKey: string }> = [
...
interface NpcEntry extends NpcData {
  targetX: number;
  targetY: number;
  idleUntilMs: number;
  minX: number;
...
export class NpcManager {
  ...
  private _stateMsg(npc: NpcEntry): ServerMessage {
    return {
      type: "npc.state",
      npcId: npc.npcId,
      x: npc.x,
