...
function drawHead(g: CanvasGraphics, s: AvatarSpec, ox: number, oy: number) {
  ...
  g.fillStyle(shade(s.skin, 0.78), 0.22);
  g.fillEllipse(ox - EX, eyeY - 0.7, 2.0, 0.8);
  g.fillEllipse(ox + EX, eyeY - 0.7, 2.0, 0.8);
  g.fillStyle(shade(s.skin, 0.82), 0.18);
  ...
  g.fillCircle(ox + EX, eyeY + 0.05, 0.54);
  g.fillStyle(shade(iris, 1.35), 0.5);
  ...
  g.fillCircle(ox + EX - 0.2, eyeY - 0.22, 0.17);
  // Upper lash line + a soft lower-lid line for eye shape.
  ...
  g.arc(ox + EX, eyeY - 0.06, 1.32, 1.02 * Math.PI, 1.98 * Math.PI, false);
  g.strokePath();
  g.lineStyle(0.32, shade(s.skin, 0.62), 0.5);
  g.beginPath();
  ...
  if (fh !== "none") {
    ...
    if (fh === "beard" || fh === "mustache") {
      g.fillStyle(beardHair, 1);
      g.fillEllipse(ox, oy - 10.5, 3.6, 1.1);
    }
  }
  ...
  if ((s.glasses ?? "none") !== "none") {
    ...
    g.fillCircle(ox - 2, eyeY, 1.8);
    g.fillCircle(ox + 2, eyeY, 1.8);
    g.lineStyle(0.7, frame, 0.95);
    g.strokeCircle(ox - 2, eyeY, 2.1);
    g.strokeCircle(ox + 2, eyeY, 2.1);
    g.fillStyle(frame, 0.9);
    g.fillRect(ox - 0.3, eyeY - 0.3, 0.6, 0.5);
    g.fillRect(ox - 4.9, eyeY - 0.3, 0.9, 0.4);
    g.fillRect(ox + 4, eyeY - 0.3, 0.9, 0.4);
  }
}
...
function drawFrontArms(g: CanvasGraphics, s: AvatarSpec, swing: number) {
  ...
  g.fillStyle(shade(s.suit, 0.6), 0.8);
...
function drawFront(g: CanvasGraphics, s: AvatarSpec) {
  ...
  g.fillStyle(shade(s.trouser, 0.55), 0.7);
  ...
  g.lineStyle(0.5, shade(s.suit, 1.5), 0.6);
  g.beginPath();
  g.moveTo(-sw + 0.6, -5.6);
  g.lineTo(sw - 0.6, -5.6);
  g.strokePath();
  ...
  g.lineTo(2.2, 1);
  g.strokePath();
  g.lineStyle(0.4, shade(s.suit, 1.5), 0.8);
  g.beginPath();
  g.moveTo(-3.0, -5.6);
  g.lineTo(-2.1, 0.6);
  g.strokePath();
  ...
  g.lineStyle(0.5, shade(s.suit, 0.55), 0.85);
  g.beginPath();
  g.moveTo(-4.9, -2.7);
  g.lineTo(-3.0, -2.7);
  g.strokePath();
  ...
  g.fillStyle(shade(s.suit, 0.45), 1);
  g.fillCircle(0, 5.0, 0.5);
  g.fillCircle(0, 6.4, 0.46);
  g.fillStyle(shade(s.suit, 1.5), 0.6);
  g.fillCircle(-0.16, 4.85, 0.18);
  g.fillCircle(-0.15, 6.26, 0.16);
...
function coinBody(
  ...
  // crisp premium rim as the humanized executives. Mirrors ExecutiveAvatar._coinBody.
  if (ring) {
    g.fillStyle(OUTLINE, 1);
