...
import { mintShareLink, type ShareScope } from "@/lib/share-links";
 ...
 *
 * The minting itself lives in `mintShareLink`, shared with the sessions
 * command bar so both surfaces reuse a share rather than each inventing their
 * own policy.
 */
export async function shareEarnConversation(
  ...
  if (!ctx?.orgId) return { ok: false, error: "Not signed in." };
  if (!sessionId) {
    return { ok: false, error: "Ask Earn something first — there's nothing to share yet." };
  }

  const supabase = await createServerClient();
  return mintShareLink(supabase, {
    orgId: ctx.orgId,
    userId: ctx.userId,
    sessionId,
    scope,
