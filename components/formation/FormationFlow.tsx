...
function FormationStep({
  ...
  }, [phase, reduced, steps.length, item.id, onCompleted]);
      ...
      <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-[minmax(0,1fr)_300px]">
        <Card className="p-6">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-2.5">
            <Button variant="ghost" icon={Bookmark} onClick={onClose}>
              Save &amp; close
            </Button>
            <Button
              variant="gold"
              iconRight={FileSignature}
              onClick={() => {
                setN(0);
                setPhase('filing');
              }}
...
function FormationComplete({
  ...
  const onRecord = [
    ...
    { name: 'Form D', sub: `${exemption} · filed with the SEC`, layer: 'Execution' },
  ...
  const unlocks = [
    ...
    },
    {
      ...
      sub: 'Every document is in one secure, trackable room for LP diligence.',
      href: '/materials'
    },
    {
      icon: GraduationCap,
      tone: 'success',
      title: 'Onboard LPs',
      sub: 'Send subscription packs and verify accreditation — fully tracked.',
      href: '/pipeline'
    }
  ];
...
export function FormationFlow({ firm, sizeLabel }: FormationFlowProps) {
  const [view, setView] = useState<'story' | 'checklist' | 'item' | 'complete'>('story');
  const [d, setD] = useState<FormationData>(FORMATION_D0);
        ...
        <div className="relative max-w-[64ch]">
          ...
          <div className="mt-4 flex items-center gap-2.5 rounded-xl border border-[var(--gold-line)] bg-[var(--gold-soft)] px-3.5 py-2.5">
            ...
            <span className="text-[12.5px] text-fg-2">
              ...
              <span className="font-semibold">“Not sure yet”</span> and Earn decides it for you,
              then flags it for review.
            </span>
          </div>
        </div>
      </Card>
