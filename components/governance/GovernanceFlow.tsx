...
function PolicyBuilder({
  ...
  const [d, setD] = useState<Record<string, PolicyValue>>(() => policyDefaults(pol));
    ...
    return (
      <div className="mx-auto flex w-full max-w-[540px] flex-col items-center py-5">
        ...
        <p className="mt-1.5 text-center text-[12.5px] text-fg-3">
          Drafted to the institutional standard and active across the firm.
        </p>
        ...
        <div className="mt-4 flex w-full flex-wrap items-center justify-between gap-2.5">
          ...
          <Button variant="gold" iconRight={Check} onClick={onAdopt}>
            Adopt &amp; finish
          </Button>
        </div>
      </div>
...
/** Fill the first open seat in a roster from a cycling candidate bench. */
function useRoster(initial: readonly GovMember[], candidates: readonly GovCandidate[]) {
  const [members, setMembers] = useState<GovMember[]>([...initial]);
  const [idx, setIdx] = useState(0);
  const add = () => {
    const cand = candidates[idx % candidates.length];
    if (!cand) return;
    setMembers((prev) => {
      const at = prev.findIndex((m) => m.open);
      if (at < 0) return prev;
      const next = [...prev];
      next[at] = {
        id: `${next[at].id}-f${idx}`,
...
export function GovernanceFlow({ firm, principal }: GovernanceFlowProps) {
  ...
  const fm = useRoster(FM_0, FM_CANDIDATES);
  const ic = useRoster(IC_MEMBERS_0, IC_CANDIDATES);
  const adv = useRoster(ADV_0, ADV_CANDIDATES);
  const cap = useRoster(CAP_0, CAP_CANDIDATES);
  const legal = useRoster(LEGAL_0, LEGAL_CANDIDATES);
    ...
    if (pol) {
      return (
        <PolicyBuilder
          ...
          onAdopt={() => {
            setAdopted((prev) => (prev.includes(pol.id) ? prev : [...prev, pol.id]));
            setOpenPol(null);
          }}
        />
      );
  ...
  const structureNodes = [
    ...
    { label: `${firm} Fund I, LP`, sub: 'Delaware LP', icon: Landmark, tone: 'gold' },
    {
      label: `${firm} GP, LLC`,
      ...
      {view === 'policies' && (
        <Card className="p-[18px]">
          ...
          <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
            {GOV_POLICIES.map((p) => {
              const active = adopted.includes(p.id);
              return (
                <div
                  key={p.id}
