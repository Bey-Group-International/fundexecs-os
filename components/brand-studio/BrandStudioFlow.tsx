...
import { Avatar, Badge, Button, Card, ProgressBar, SegTabs } from '@/components/ui';
...
/* ── the copiloted brand builder (inline full-view swap) ─────────────────── */
...
function BrandBuilder({
  ...
  id: string;
  onBack: () => void;
  onComplete: (spec: Record<string, BrandValue>) => void;
}) {
  const reduced = useReducedMotion() ?? false;
  const cfg: BrandBuildCfg = BRAND_BUILD[id];
  const name = BRAND_ITEM_NAME[id] ?? id;
  const [d, setD] = useState<Record<string, BrandValue>>(() => brandDefaults(cfg));
  ...
  if (phase === 'done') {
    return (
      <div className="mx-auto flex w-full max-w-[560px] flex-col items-center py-5">
        ...
        <h2 className="text-[20px] font-semibold tracking-[-0.015em] text-fg-1">{name} — live</h2>
        <p className="mt-1.5 text-center text-[13px] text-fg-3">
          Produced to your spec and published across your workspace.
        </p>
        ...
        <div className="mt-5 flex w-full flex-wrap items-center justify-between gap-2.5">
          ...
          <Button variant="gold" iconRight={Check} onClick={() => onComplete(d)}>
            Publish &amp; finish
          </Button>
        </div>
      </div>
        ...
        <Card className="self-start p-[17px]">
          <div className="mb-3 flex items-center gap-2.5">
            <div className="relative flex-none">
              <EarnCoin size={32} />
              <span className="absolute -bottom-px -right-px h-2.5 w-2.5 rounded-full border-2 border-bg-2 bg-success" />
            </div>
            <div>
...
export function BrandStudioFlow({ firm, principal }: { firm: string; principal: string }) {
  const [view, setView] = useState<'profile' | 'brand' | 'presence'>('profile');
  const [openId, setOpenId] = useState<string | null>(null);
  const [built, setBuilt] = useState<Record<string, Record<string, BrandValue>>>({});
  ...
  const tagline = kitBuilt ? 'Backing the operators others overlook.' : null;
      ...
      {view === 'profile' ? (
            ...
            <div className="flex flex-wrap items-start gap-4">
              ...
              <div className="min-w-[200px] flex-1">
                ...
                <div className="mt-2.5 max-w-[62ch] text-[12px] leading-relaxed text-fg-2">
                  {bioBuilt ? (
                    `${principal} is the Managing Partner of ${firm} — an operator-turned-investor who built in the sector before backing it, with a track record and a focused thesis LPs can underwrite.`
                  ) : (
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {['LinkedIn', 'X', 'Website'].map((s) => (
                    <span
                      key={s}
                      className={cn(
                        'inline-flex items-center gap-1.5 rounded-full border border-hairline bg-surface-1 px-2.5 py-1 text-[11px]',
                        connected[s.toLowerCase()] ? 'text-success' : 'text-fg-4'
                      )}
                    >
                      {connected[s.toLowerCase()] ? (
                ...
                >
                  ...
                  {it.done ? (
                    ...
                    <Button
                      ...
                      size="sm"
                      icon={Plus}
                      onClick={() => setConnected((p) => ({ ...p, [it.id]: true }))}
                    >
                      Build
                    </Button>
                  )}
        ...
        <Card className="p-[18px]">
          ...
          >
            ...
            >
              {tagline ?? 'Your tagline appears once your brand kit is built.'}
            </div>
          </div>
            ...
            <div className="flex items-center gap-2.5 rounded-[12px] border border-hairline bg-surface-1 px-3.5 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-[10.5px] text-fg-5">
                  {siteBuilt
                    ? `${firm.toLowerCase().replace(/[^a-z]/g, '')}.com · live`
      ...
      {(view === 'profile' || view === 'presence') && (
        <Card className="p-[18px]">
          <PanelHeader
            icon={Plug}
            title="Connections"
            eyebrow="One-click integrations · keep everything in sync"
          />
            ...
            {CONNECTORS.map((c) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  aria-pressed={on}
                  onClick={() => setConn((p) => ({ ...p, [c.id]: !p[c.id] }))}
                  className={cn(
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      on ? 'text-success' : 'text-[var(--accent)]'
                    )}
                  >
