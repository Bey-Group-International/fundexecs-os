...
function stageBadgeClass(stage: string): string {
  ...
  if (s.includes("close") || s.includes("sign")) return "bg-status-success/10 text-status-success border-status-success/40";
  if (s.includes("due") || s.includes("diligen")) return "bg-gold-500/10 text-gold-300 border-gold-500/40";
  if (s.includes("prospect") || s.includes("sourcing")) return "bg-surface-2 text-fg-secondary border-line";
  return "bg-surface-2 text-fg-secondary border-line";
}
...
export function StaleDealAlerts({ deals }: { deals: StaleDeal[] }) {
  ...
  const severityBands = [
    { label: "Critical", sublabel: ">90d", count: critical, color: "text-status-danger", bar: "bg-status-danger" },
    { label: "High", sublabel: ">60d", count: high, color: "text-status-warning", bar: "bg-status-warning" },
    { label: "Medium", sublabel: ">30d", count: medium, color: "text-gold-300", bar: "bg-gold-500" },
    { label: "Low", sublabel: "<30d", count: low, color: "text-fg-muted", bar: "bg-fg-muted" },
  ];
