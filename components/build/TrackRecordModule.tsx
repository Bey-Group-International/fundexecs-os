...
import { TrackRecordAnalytics, SortableTrackRecordList } from "./TrackRecordAnalytics";
...
export async function TrackRecordModule() {
  ...
  // Pre-render each row server-side; the client list re-orders by id.
  ...
  return (
    <div>
      ...
      {records.length === 0 ? (
        ...
        <>
          <TrackRecordAnalytics records={records} />
          <SortableTrackRecordList
            records={records}
            rows={records.map((r) => ({ id: r.id, node: rows.get(r.id) }))}
          />
        </>
      )}
    </div>
